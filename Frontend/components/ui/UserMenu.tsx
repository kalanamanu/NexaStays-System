...
export default function UserMenu({ compact = false }: { compact?: boolean }) {
  ...
  // ---- Clerk and Manager: Only show logout ----
    ...
    return (
      <div className="relative inline-block text-left" ref={menuRef}>
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen((s) => !s)}
          className={`inline-flex items-center space-x-3 rounded-full px-2 py-1 focus:outline-none focus:ring-2 focus:ring-white/40 transition text-sm`}
        >
  ...
  // For travel-company, do NOT include profile
  if (user.role !== "travel-company") {
    menuItems.push({
      label: "Profile",
